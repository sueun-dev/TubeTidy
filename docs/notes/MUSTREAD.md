...
## 2026-02-07 Python 린트 도구 실행 오류 기록 (ruff)

### 발생한 오류
- 명령어: `./.venv/bin/python -m ruff check server/app.py`
- 오류: `No module named ruff`

### 왜 발생했는가
- 현재 `.venv`에는 `ruff`가 설치되어 있지 않다.
- 이 저장소의 기본 검증 체인은 `flutter analyze/test`, `unittest`, `bandit`, `pip-audit`이며 `ruff`는 기본 의존성이 아니다.

### 다시는 같은 실수를 하지 않기 위한 규칙
- 임의 린트 도구 실행 전 설치 여부를 먼저 확인한다.
- 설치되어 있지 않은 도구는 즉시 대체 검증(기본 체인)으로 전환한다.

### 표준 실행 절차
1. `./.venv/bin/python -m pip show ruff` 확인
2. 없으면 프로젝트 기본 검증 체인 실행:
   - `./.venv/bin/python -m unittest discover -s server/tests -v`
   - `./.venv/bin/bandit -q -r server`
   - `./.venv/bin/pip-audit -r server/requirements.txt`
